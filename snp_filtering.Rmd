...
- This script invokes two system commands to run `blastn` and `bedtools`. The full path to working versions of these programs should be specfied in the script accordingly.
- The packages listed below are required. NOTE: The development version of dplyr is required.
...
- For SNPs called from the `original` transcriptome (18,971), calculate mean *MAF* and *depth*
- For SNPs called from the `illumina` transcriptome (20,426), calculate mean *MAF* and *depth*
- Repeat these steps for the SNPs called by both methods: `intersect` (4,679)
...
- Repeat these steps for the SNPs called from both illumina and 454 sequencing data (4679). Code hidden in output.
...
The files returned by each manufacturer are stored in `data/`
...
# Files I already merged earlier (using blast against the full genome)
